...
try:
    ...
    # 验证id是否确实为年龄最小学生
    actual_min_age_student = min(students.items(), key=lambda x: x[1][3])
    if min_age_id == actual_min_age_student[1][0]:
        add_point(10, "get_id_by_min_age success, found min age student id: {}".format(min_age_id))
    else:
        print(type(min_age_id), type(actual_min_age_student[1]))
        print_error("get_id_by_min_age failed, found: {}, expected {}".format(min_age_id, actual_min_age_student[0]))
except:
    print("get_id_by_min_age error!")


# 测试更新指定学生院系
print_info("\n[Test update_dept_by_id]")
update_id_list = random.sample(all_id, 1)
update_id = update_id_list[0]
new_dept = "数据学院"
...
try:
    ...
    if tx_receipt is not None:
        ...
        if len(update_event) > 0:
            # 从事件中获得更新的学号和院系信息
            updated_id = update_event.get('args').get('id')
            updated_dept = update_event.get('args').get('dept')
            print(updated_id, updated_dept)
            if updated_id == update_id and updated_dept == new_dept:
                add_point(10, "update_dept_by_id success! updated id: {}, updated dept: {}".format(updated_id, updated_dept))
            else:
                print_error("update_dept_by_id failed! updated id: {}, updated dept: {}".format(updated_id, updated_dept))
        else:
            print_error("update_dept_by_id failed! event not found")
    else:
        print_error("update_dept_by_id failed! transaction receipt is none")
except Exception as e:
