...
public class MinerNode extends Thread {
    ...
    public BlockBody getBlockBody(Transaction[] transactions) {
        ...
        for(int i = 0; i < transactions.length; i ++){
            String t = SHA256Util.sha256Digest(transactions[i].toString());
            primaryHash.add(t);
        }
    ...
    public List<String> getHashList(List<String> hashValue){
        ...
        while(index < l){
            ...
            if(index != l){
                ...
                // 两两加密
                String shaValue = SHA256Util.sha256Digest(left + right);
                newHashList.add(shaValue);
                index ++;
            }
            else{
                String shaValue = SHA256Util.sha256Digest(left + left);
                newHashList.add(shaValue);
            }
        }
        return newHashList;
    ...
    public Block getBlock(BlockBody blockBody) {
        ...
        String newHash = SHA256Util.sha256Digest(lastBlock.toString());
