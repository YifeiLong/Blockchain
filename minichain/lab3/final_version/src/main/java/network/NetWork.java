...
//        transactionProducer = new TransactionProducer(this);
    ...
    public void theyHaveADayDream(){
        UTXO[] outUtxo = new UTXO[accounts.size()];
        // 在创世块中为每个账户分配一定金额的UTXO，便于后续交易进行
        for(int i = 0; i < accounts.size(); ++i){
            outUtxo[i] = new UTXO(accounts.get(i).getWalletAddress(), MiniChainConfig.INIT_AMOUNT,
...
//        transactionProducer.start();
    ...
    public Account create_account(){
        ...
        Transaction transaction = new Transaction(new UTXO[]{}, outUtxos, sign, dayDreamPublicKey,
